...
When the first `.so` is introduced, add a release-build check (e.g. `readelf -l` or Play pre-launch report). v0.1.6 has no native libs; `./gradlew assembleRelease` is sufficient for current alpha builds.